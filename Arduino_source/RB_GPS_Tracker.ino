/* RazorBlock BlockChain Hackathon project 2022
 * Using GPS example code from RAK github page, copied on 3-24-22 
 * Making changes to support the hackathon project
 * The RazorBlock team is using as a POC for GPS tracking
 * 
 * Code listed on RAK github page is posted with the MIT license protocol
 */
...
/************************** defines inserted or moved by Witty to support Hackathon *****************/
#define LORAWAN_APP_INTERVAL 10000    /**< Defines for user timer, the application data transmission interval. 10s, value in [ms]. Default 10000, Witty moved for access*/
#define TRUE 1
#define FALSE 0
#define SKIP_ACL_CHECK TRUE           // Skip original code accelerometer check if set to TRUE, try to send GPS every interval
/***************************** above defines added or moved by Witty to support Hackathon *****************/
...
#define LORAWAN_DATERATE DR_3 //Witty making this DR_3 per WW suggestion to solve connection problem
#define LORAWAN_TX_POWER TX_POWER_5             /*LoRaMac tx power definition, from TX_POWER_0 to TX_POWER_15*/
#define JOINREQ_NBTRIALS 3                      /**< Number of trials for the join request. */
DeviceClass_t gCurrentClass = CLASS_A;          /* class definition*/
LoRaMacRegion_t gCurrentRegion = LORAMAC_REGION_US915;    /*Witty changed to US915 was Region:EU868*/
...
//Witty change making this LORAWAN_ADR_OFF per WW suggestion for Helim connection issue
...
//Witty moving this line to top for easy access to timing
...
bool send_flag=false; /*Witty added to flag when good send happend */
...
/* Witty - setting up LED for case output */
/* IO3 is on the 5804 breadout module, ground is pin 2 and IO3 is pin 7, pin 1 is on the outter front edge of board and next to USB connector */
...
/*************************************/  
  ...
  Serial.println("RazorBlock team hackathon entry V11");
...
void loop()
{
  ...
  {
    send_flag=false;
    flash_external_led2(); /*Witty added - success flash*/
  }
}
...
static void lorawan_join_failed_handler(void)
{
  ...
  while(true) { flash_external_led(); delay(1000); } /* Witty added, failure flash */
...
void send_lora_frame(void)
{
  ...
  if (error == LMH_SUCCESS)
  {
    ...
    send_flag=true;  /*Witty - flag a good send for flash */
  }
  else
  {
    count_fail++;
...
{ 
  ...
  Serial.println(data);
  data = "";
  if( abs(x-z) < 400 || SKIP_ACL_CHECK)
  {
    Serial.println("Calling flash routine");
    flash_external_led();  //Witty, flash external LED
    Serial.println("Back from flash routine");
    ...
    for (unsigned long start = millis(); millis() - start < 1000;)
    {
      ...
      {
        char c = Serial1.read();
         //Witty uncommented to see raw GPS data
    ...
    {
      Serial.println("\n*********************** No Location Found");  /*Witty added new line*/
      TimerSetValue(&appTimer, LORAWAN_APP_INTERVAL);
      TimerStart(&appTimer);
    }
  }
...
/* Witty - routine to flash external LED on case */
void flash_external_led(void)
{
  for (int a=0;a<25;a++)
  {
   delay(100);
